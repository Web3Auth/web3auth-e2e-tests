...
export class WalletServicesPage {
    ...
    async verifyBalanceAndAddress(address: string, balance: string) {
        ...
        let walletBalance = await this.page
            .locator(`xpath=.//p[contains(text(),'Total Wallet Balance')]/parent::div/div/p`)
            .first()
            .textContent();
    ...
    async verifyTransactionActivity(details: string) {
        ...
        for (const part of parts) {
            console.log(part);
            expect(
                await block.locator(`xpath=.//p[contains(text(),"${part}")]`).last().isVisible()
            ).toBeTruthy();
        }
    }
    ...
    async selectNetwork(currentNetwork: string, newNetwork: string) {
        await this.page
            .locator(`xpath=.//p[text()='Select Default Network']/parent::div//button`)
            .first()
            .click();
    ...
    async selectCurrency(currency: string) {
        await this.page
            .locator(`xpath=.//p[text()='Select Default Currency']/parent::div//button`)
            .first()
            .click();
    ...
    async enterRecipientAddress(address: string) {
        await this.page
            .locator(`xpath=.//input[@placeholder='Enter recipient address']`)
            .fill(address);
    }
    ...
    async verifyUserRole(email: string, role: string) {
        expect(
            await this.page
                .locator(`xpath=.//p[text()="${email}"]/ancestor::td/following-sibling::td/div`)
                .first()
                .textContent()
        ).toContain(role);
    }
    ...
    async verifyProject(name: string, environment: string, platform: string) {
        ...
        expect(await this.page.locator("#EnvironmentNetwork").first().inputValue()).toContain(
            environment
        );
        expect(
            await this.page
    ...
    async verifyUserInfoInDemoApp(email: string) {
        ...
        expect(
            await this.page
                .locator(`xpath=.//h1[text()='User Info']/parent::div//pre`)
                .first()
                .textContent()
        ).toContain(email);
    }
    ...
    async verifyAddressInDemoApp(address: string) {
        expect(
            await this.page.locator(`xpath=.//p[text()='${address}']`).first().textContent()
        ).toContain(address);
    }
    ...
    async verifyBalanceInDemoApp(balance: string) {
        ...
        expect(
            await this.page
                .locator(`xpath=.//h1[text()='Balance']/parent::div//pre`)
                .first()
                .textContent()
        ).toContain(balance);
    }
    ...
    async verifyWalletInDemoApp(address: string) {
        ...
        expect(
            await frame?.locator(`xpath=.//span[text()='${address}']`).first().textContent()
        ).toContain(address);
        expect(
            await frame
                ?.locator(`xpath=.//p[contains(@class,'wallet:ml-2')][contains(text(),'Send')]`)
                .last()
                .isVisible()
        ).toBeTruthy();
        expect(
            await frame
                ?.locator(`xpath=.//p[contains(@class,'wallet:ml-2')][contains(text(),'Receive')]`)
                .last()
                .isVisible()
        ).toBeTruthy();
        expect(
            await frame?.locator(`xpath=.//img[contains(@src,'eth')]`).first().isVisible()
        ).toBeTruthy();
        await frame
            ?.locator(`xpath=.//button[contains(@class,'absolute right-1')]`)
            .first()
            .click();
    }
    ...
    async verifyWalletConnect() {
        ...
        expect(
            await frame
                ?.locator(`xpath=.//input[@aria-placeholder='Paste QR link here']`)
                .last()
                .isVisible()
        ).toBeTruthy();
    ...
    async verifySignedMessages(signType: string, browser: Browser) {
        ...
        expect(
            await this.page
                .locator(`xpath=.//h1[text()='Success']/parent::div//pre`)
                .first()
                .textContent()
        ).toContain("signedMessage");
    }

    async verifyEncryptionAndDecryption(browser: Browser) {
        await this.page
            .locator(`xpath=.//textarea[@aria-placeholder='Message to encrypt']`)
            .fill("test message to encrypt");
        ...
        expect(
            await this.page
                ...
                .first()
                .textContent()
        ).toContain("test message to encrypt");
    }
