...
export class WalletServicesPage {
  ...
  async verifyWalletInDemoApp(address: string) {
    ...
    expect(
      await frame
        ?.locator(
          `xpath=.//p[contains(@class,'wallet:ml-2')][contains(text(),'Send')]`
        )
        .last()
        .isVisible()
    ).toBeTruthy();
    expect(
      await frame
        ?.locator(
          `xpath=.//p[contains(@class,'wallet:ml-2')][contains(text(),'Receive')]`
        )
        .last()
        .isVisible()
    ).toBeTruthy();
  ...
  async verifyWalletConnect() {
    ...
    await frame
      ?.locator(`xpath=.//button[contains(@class,'absolute top-6')]`)
      .last()
      .click();
  }
