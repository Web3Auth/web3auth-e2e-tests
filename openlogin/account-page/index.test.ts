...
test.describe("On homepage,", () => {
  ...
  test.only(`title should be "OpenLogin"`, async ({ page }) => {
    const title = await page.title();
    expect(title).toBe("OpenLogin");
  });
  test.only(`there should be "Get Started" button`, async ({ page }) => {
    expect(await page.isVisible(`button:has-text("Get Started")`)).toBeTruthy();
  });

  test.only(`click on "Get Started" should show login modal`, async ({
    page,
  }) => {
